  ...
  try {
    ...
    if (!keyId || !keySecret) {
      return NextResponse.json(
        { success: false, error: "Razorpay credentials are missing in environment configuration." },
        { status: 500 }
      );
    }

    // Convert INR to Paise if provided in INR, or ensure minimum 100 paise
    const amountInPaise = amount < 100 ? Math.round(Number(amount) * 100) : Math.round(Number(amount));
