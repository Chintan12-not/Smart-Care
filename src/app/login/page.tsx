...
function LoginContent() {
  ...
  const { user, signIn, signUp, signInWithGoogle, signInWithApple, loading: authLoading } = useAuth();
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.error || "Failed to connect to Apple.");
      }
    } catch (err: any) {
      setError(err.message || "An unexpected error occurred during Apple Auth.");
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-3">
            <button
              ...
              disabled={loading || authLoading}
              className="w-full flex items-center justify-center gap-3 py-2.5 px-4 rounded-xl border border-border bg-muted/30 hover:bg-muted/70 transition-all duration-200 active:scale-[0.98] text-xs font-bold text-foreground"
            >
